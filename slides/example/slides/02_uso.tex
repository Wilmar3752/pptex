...
\begin{frame}[fragile]{Flujo de trabajo completo}
  \begin{center}
    \begin{tikzpicture}[
      node distance=2.6cm,
      step/.style={rectangle, draw, rounded corners=5pt,
                   minimum width=2.8cm, minimum height=1cm,
                   fill=pptexBlue!15, font=\small\bfseries},
      lbl/.style={font=\tiny\ttfamily, text=gray, below=2pt},
    ]
      \node[step] (new)  {new};
      \node[step] (edit) [right of=new]    {editar};
      \node[step] (comp) [right of=edit]   {compile};
      \node[step] (pdf)  [right of=comp]   {PDF};
...
\begin{frame}{Motores disponibles}
  \begin{table}
    ...
    \begin{tabular}{lll}
      ...
      \midrule
      XeLaTeX     & (default)         & Siempre, soporta Unicode y fuentes del sistema \\
      LuaLaTeX    & \texttt{-{}-engine lualatex} & Programacion avanzada con Lua \\
      pdfLaTeX    & \texttt{-{}-engine pdflatex} & Documentos legacy o maxima velocidad \\
      \bottomrule
    \end{tabular}
  \end{table}
